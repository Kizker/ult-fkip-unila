...
@endphp
<div class="page-admin-cms page-admin-cms-posts-index page-admin-cms-blogs-index" data-cms-page="blogs-index">
  <header class="cms-page-header">
    <div class="cms-page-heading">
      <h1 class="cms-page-title">Blog</h1>
      <p class="cms-page-subtitle">Kelola konten blog publik.</p>
    </div>
    <div class="cms-page-actions">
      <x-button href="{{ route('admin.cms.blogs.create') }}">Tambah</x-button>
  ...
  <x-card class="cms-card admin-search-card" data-admin-search-card>
    <div class="admin-search" role="search" aria-label="Pencarian blog">
      <div class="admin-search__toolbar">
        <div class="admin-search__field">
          <label for="admin-cms-blogs-live-search" class="sr-only">Cari blog</label>
          <div class="admin-search__input-wrap">
            <input
              id="admin-cms-blogs-live-search"
              type="text"
            ...
            >
            <button type="button" class="admin-search__clear admin-search__clear--disabled" aria-label="Hapus pencarian" data-admin-search-clear data-reset-url="{{ route('admin.cms.blogs.index') }}">
    ...
    <div class="admin-search-resultbar">
      <div class="admin-search-resultbar__count admin-search-resultbar__count--live" data-realtime-search-count data-default-count-text="Menampilkan {{ $shown }} dari {{ $total }} blog">Menampilkan {{ $shown }} dari {{ $total }} blog</div>
    </div>
        ...
        <tbody>
            ...
            <tr class="border-t border-[rgb(var(--c-border))]" data-realtime-search-item data-realtime-search-text="{{ trim($it->title_id . ' ' . $it->slug . ' ' . ($it->is_published ? 'published' : 'draft')) }}">
              ...
              <td class="py-2 text-right" data-label="Aksi">
                <div class="cms-row-actions inline-flex gap-2">
                  <x-button href="{{ route('admin.cms.blogs.edit',$it) }}" variant="ghost">Edit</x-button>
                  <form method="post" action="{{ route('admin.cms.blogs.destroy',$it) }}">
                    @csrf @method('DELETE')
                    <x-button type="submit" variant="danger" data-confirm="Hapus blog ini?">Hapus</x-button>
                  </form>
                </div>
              </td>
            </tr>
          @empty
            <tr class="border-t border-[rgb(var(--c-border))]">
              <td class="py-6 text-center text-muted" colspan="4">Belum ada blog.</td>
            </tr>
          @endforelse
          @if($items->count() > 0)
            <tr class="border-t border-[rgb(var(--c-border))] hidden" data-realtime-search-empty>
              <td class="py-6 text-center text-muted" colspan="4">Tidak ada blog yang cocok dengan pencarian.</td>
            </tr>
          @endif
        </tbody>
      </table>
