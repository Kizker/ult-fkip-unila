...
<div class="page-admin-doc-formats-create">
  <header class="admin-page-header">
    <div class="admin-page-heading">
      ...
      <h1 class="admin-page-title">Tambah Format Nomor</h1>
      <p class="admin-page-subtitle">Buat template nomor dokumen per unit dan per <span class="dn-mono">format_key</span>.</p>
    </div>
  ...
  <div class="dn-form-layout">
    <x-card class="dn-form-card">
      <form method="POST" action="{{ route('admin.doc_formats.store') }}" class="dn-form">
        @csrf
          ...
          <div class="dn-form__grid">
            <div class="dn-field">
              ...
              <select name="unit_id" required class="dn-select">
                @foreach($units as $u)
                  <option value="{{ $u->id }}">{{ $u->type->value }} &mdash; {{ $u->name }} ({{ $u->code }})</option>
                @endforeach
              </select>
            </div>
          </div>
        ...
        <div class="dn-form__section">
          ...
          <div class="dn-form__grid dn-form__grid--2">
            <div class="dn-field">
              <x-input name="format_key" label="Format Key" value="{{ old('format_key', $defaults['format_key']) }}" required />
              <p class="dn-hint">Contoh: <span class="dn-mono">default</span>, <span class="dn-mono">rekomendasi</span>, <span class="dn-mono">surat_pengantar</span></p>
            </div>
            <div class="dn-field">
              <x-input name="name" label="Nama" value="{{ old('name', $defaults['name']) }}" required />
            </div>
          </div>
        </div>
        ...
        <div class="dn-form__section">
          ...
          <div class="dn-form__grid">
            <div class="dn-field">
              <x-textarea name="template" label="Template" rows="3" required>{{ old('template', $defaults['template']) }}</x-textarea>
              <p class="dn-hint">Wajib mengandung <span class="dn-mono">{SEQ}</span> atau <span class="dn-mono">{SEQ:n}</span>.</p>
            </div>
          </div>
        </div>
        ...
        <div class="dn-form__section">
          ...
          <div class="dn-form__grid dn-form__grid--2 dn-form__grid--tight">
            <div class="dn-field">
              <x-input type="number" name="seq_padding" label="Padding default" value="{{ old('seq_padding', $defaults['seq_padding']) }}" min="1" max="10" />
            </div>
            <label class="dn-switch">
              <input type="checkbox" name="is_active" value="1" checked class="dn-switch__box">
    ...
    <div class="dn-aside">
      <x-card>
        <div class="admin-card-title">Panduan cepat</div>
        <div class="admin-card-subtitle">Gunakan placeholder untuk membentuk nomor.</div>
