...
<div class="page-admin-dashboard-v2" x-data="dashboardFilter()" x-init="init()">
  ...
  <header class="dash-header">
    ...
    <div class="dash-header__actions">
      ...
      <div class="dash-period-label" x-text="periodLabel">{{ $range['label'] }}</div>
    </div>
  </header>
  ...
  <section class="dash-summary" aria-label="Ringkasan">
    <div class="dash-summary__card dash-summary__card--violet" id="summaryTotal">
      ...
      <div class="dash-summary__data">
        <div class="dash-summary__value" x-text="summary.total">{{ $summary['total'] }}</div>
    ...
    <div class="dash-summary__card dash-summary__card--emerald" id="summaryCompleted">
      ...
      <div class="dash-summary__data">
        <div class="dash-summary__value" x-text="summary.completed">{{ $summary['completed'] }}</div>
    ...
    <div class="dash-summary__card dash-summary__card--amber" id="summaryPending">
      ...
      <div class="dash-summary__data">
        <div class="dash-summary__value" x-text="summary.pending">{{ $summary['pending'] }}</div>
    ...
    <div class="dash-summary__card dash-summary__card--rose" id="summaryRejected">
      ...
      <div class="dash-summary__data">
        <div class="dash-summary__value" x-text="summary.rejected">{{ $summary['rejected'] }}</div>
  ...
  <section class="dash-charts" aria-label="Grafik"
    ...
    <div class="dash-chart-card dash-chart-card--trend">
      <div class="dash-chart-card__header">
        ...
        <span class="dash-chart-card__badge" x-text="periodLabel">{{ $range['label'] }}</span>
      </div>
  ...
  {{-- ═══════════════════════════════════════════════════════
       ANTRIAN TERBARU (Revamp) + KPI STATUS (Revamp)
  ═══════════════════════════════════════════════════════ --}}
  <section class="dash-insights">
    ...
    <div class="dash-queue-card">
      <div class="dash-queue-card__header">
        <div>
          ...
          <p class="dash-queue-card__subtitle">{{ $queueCount }} permohonan terbaru yang perlu diproses</p>
        </div>
        <a href="{{ route('admin.requests.index') }}" class="dash-link-btn">
          Lihat semua
      ...
      <div class="dash-queue-list">
        @forelse($queue as $r)
          <a href="{{ route('admin.requests.show', $r) }}" class="dash-queue-item" id="queueItem{{ $r->id }}">
            ...
            <div class="dash-queue-item__body">
              <div class="dash-queue-item__title">{{ $r->display_title }}</div>
              <div class="dash-queue-item__meta">
                <span>{{ $r->student->name ?? '-' }}</span>
                <span class="dash-queue-item__dot">&bull;</span>
                <time datetime="{{ $r->created_at->toIso8601String() }}">{{ $r->created_at->diffForHumans() }}</time>
              </div>
            </div>
    ...
    <div class="dash-kpi-card">
      <div class="dash-kpi-card__header">
        ...
        <span class="dash-kpi-card__total">{{ $totalAll }} total</span>
      </div>
      <div class="dash-kpi-grid">
        @foreach($kpi as $item)
          <a href="{{ route('admin.requests.index', ['status' => $item['status']]) }}"
             class="dash-kpi-item"
             style="--kpi-color: {{ $item['color'] }}"
             aria-label="Lihat permohonan {{ $item['label'] }}">
            <div class="dash-kpi-item__top">
              <x-status-badge :status="$item['status']" />
              <span class="dash-kpi-item__count">{{ $item['count'] }}</span>
            </div>
            <div class="dash-kpi-item__bar-track">
              <div class="dash-kpi-item__bar-fill" style="width: {{ $item['percentage'] }}%; background: {{ $item['color'] }}"></div>
            </div>
            <div class="dash-kpi-item__pct">{{ $item['percentage'] }}%</div>
          </a>
        @endforeach
      </div>
    </div>
    ...
    <div class="dash-table-card">
      <div class="dash-table-card__header">
        ...
        <div class="dash-table-actions">
          <form action="{{ route('admin.dashboard') }}" method="GET" class="dash-table-search">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau layanan..." class="dash-table-search__input" id="tableSearch">
          </form>
          <div class="dash-export-dropdown" x-data="{ open: false }">
            ...
            <div class="dash-export-menu" x-show="open" x-on:click.away="open = false" x-transition>
              <a href="{{ route('admin.dashboard.export', ['format' => 'csv', 'period' => $period, 'from' => request('from'), 'to' => request('to')]) }}" class="dash-export-menu__item">
              ...
              <a href="{{ route('admin.dashboard.export', ['format' => 'excel', 'period' => $period, 'from' => request('from'), 'to' => request('to')]) }}" class="dash-export-menu__item">
                <svg viewBox="0 0 20 20" fill="currentColor" width="16" height="16"><path fill-rule="evenodd" d="M4.5 2A1.5 1.5 0 003 3.5v13A1.5 1.5 0 004.5 18h11a1.5 1.5 0 001.5-1.5V7.621a1.5 1.5 0 00-.44-1.06l-4.12-4.122A1.5 1.5 0 0011.378 2H4.5z" clip-rule="evenodd"/></svg>
                Excel
              </a>
              <a href="{{ route('admin.dashboard.export', ['format' => 'pdf', 'period' => $period, 'from' => request('from'), 'to' => request('to')]) }}" class="dash-export-menu__item">
                <svg viewBox="0 0 20 20" fill="currentColor" width="16" height="16"><path fill-rule="evenodd" d="M4.5 2A1.5 1.5 0 003 3.5v13A1.5 1.5 0 004.5 18h11a1.5 1.5 0 001.5-1.5V7.621a1.5 1.5 0 00-.44-1.06l-4.12-4.122A1.5 1.5 0 0011.378 2H4.5z" clip-rule="evenodd"/></svg>
                PDF
              </a>
            </div>
        ...
        <table class="dash-table" id="transactionsDesktopTable">
          <thead>
            <tr>
              <th>No</th>
              <th>
                <a href="{{ route('admin.dashboard', array_merge(request()->query(), ['sort' => 'id', 'dir' => request('sort') === 'id' && request('dir') === 'asc' ? 'desc' : 'asc'])) }}" class="dash-table__sort">
                  Kode
                  @if(request('sort') === 'id')
                    <svg viewBox="0 0 20 20" fill="currentColor" width="12" height="12"><path fill-rule="evenodd" d="{{ request('dir') === 'asc' ? 'M14.77 12.79a.75.75 0 01-1.06-.02L10 8.832 6.29 12.77a.75.75 0 01-1.08-1.04l4.25-4.5a.75.75 0 011.08 0l4.25 4.5a.75.75 0 01-.02 1.06z' : 'M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z' }}" clip-rule="evenodd"/></svg>
                  @endif
                </a>
              </th>
              ...
              <th>Layanan</th>
              <th>
                <a href="{{ route('admin.dashboard', array_merge(request()->query(), ['sort' => 'current_status', 'dir' => request('sort') === 'current_status' && request('dir') === 'asc' ? 'desc' : 'asc'])) }}" class="dash-table__sort">
                  Status
                  @if(request('sort') === 'current_status')
                    <svg viewBox="0 0 20 20" fill="currentColor" width="12" height="12"><path fill-rule="evenodd" d="{{ request('dir') === 'asc' ? 'M14.77 12.79a.75.75 0 01-1.06-.02L10 8.832 6.29 12.77a.75.75 0 01-1.08-1.04l4.25-4.5a.75.75 0 011.08 0l4.25 4.5a.75.75 0 01-.02 1.06z' : 'M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z' }}" clip-rule="evenodd"/></svg>
                  @endif
                </a>
              </th>
              <th>
                <a href="{{ route('admin.dashboard', array_merge(request()->query(), ['sort' => 'created_at', 'dir' => request('sort', 'created_at') === 'created_at' && request('dir', 'desc') === 'desc' ? 'asc' : 'desc'])) }}" class="dash-table__sort">
                  Tanggal
                  @if(request('sort', 'created_at') === 'created_at')
                    <svg viewBox="0 0 20 20" fill="currentColor" width="12" height="12"><path fill-rule="evenodd" d="{{ request('dir', 'desc') === 'asc' ? 'M14.77 12.79a.75.75 0 01-1.06-.02L10 8.832 6.29 12.77a.75.75 0 01-1.08-1.04l4.25-4.5a.75.75 0 011.08 0l4.25 4.5a.75.75 0 01-.02 1.06z' : 'M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z' }}" clip-rule="evenodd"/></svg>
                  @endif
                </a>
              </th>
              <th>Aksi</th>
          ...
          <tbody>
            @forelse($transactions as $i => $t)
              <tr>
                ...
                <td class="dash-table__code">{{ $t->request_code }}</td>
                <td>{{ $t->student->name ?? '-' }}</td>
                <td>{{ $t->service->title_id ?? '-' }}</td>
                <td><x-status-badge :status="$t->current_status->value ?? $t->current_status" /></td>
                <td>{{ $t->created_at->format('d/m/Y H:i') }}</td>
                <td>
                  <a href="{{ route('admin.requests.show', $t) }}" class="dash-table__detail-btn">Detail</a>
                </td>
              </tr>
            @empty
              <tr>
        ...
        @forelse($transactions as $i => $t)
          <div class="dash-table-mobile__card">
            <div class="dash-table-mobile__row">
              ...
              <span class="dash-table-mobile__value font-semibold">{{ $t->request_code }}</span>
            </div>
            <div class="dash-table-mobile__row">
              <span class="dash-table-mobile__label">Nama</span>
              <span class="dash-table-mobile__value">{{ $t->student->name ?? '-' }}</span>
            </div>
            <div class="dash-table-mobile__row">
              <span class="dash-table-mobile__label">Layanan</span>
              <span class="dash-table-mobile__value">{{ $t->service->title_id ?? '-' }}</span>
            </div>
            <div class="dash-table-mobile__row">
              <span class="dash-table-mobile__label">Status</span>
              <span class="dash-table-mobile__value"><x-status-badge :status="$t->current_status->value ?? $t->current_status" /></span>
            </div>
            <div class="dash-table-mobile__row">
              <span class="dash-table-mobile__label">Tanggal</span>
              <span class="dash-table-mobile__value">{{ $t->created_at->format('d/m/Y H:i') }}</span>
            </div>
            <a href="{{ route('admin.requests.show', $t) }}" class="dash-table-mobile__action">Lihat Detail →</a>
          </div>
        @empty
          <div class="dash-empty">
