...
@section('content')
<div class="page-admin-users-edit page-admin-user-form">
  <header class="admin-page-header">
    <div class="admin-page-heading">
      <div class="admin-page-kicker">Master data</div>
      <h1 class="admin-page-title">Edit User</h1>
      <p class="admin-page-subtitle">Ubah data user, foto profil, dan role.</p>
    </div>
  ...
  <form class="au-form" method="POST" action="{{ route('admin.users.update',$user) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="au-layout grid gap-4 lg:grid-cols-12 items-start">
      <x-card class="au-main-card lg:col-span-7">
        <div class="au-section-head flex flex-col gap-1 mb-4">
          ...
          <div class="text-xs text-muted">Ubah identitas, unit, dan password (opsional).</div>
        </div>
        ...
        <div class="au-grid grid gap-4 lg:grid-cols-2">
          ...
          <div class="au-span-2 lg:col-span-2">
            <x-input name="password" type="password" label="Password (kosongkan jika tidak diubah)" />
            <p class="text-xs text-muted mt-1">Jika diisi, password lama akan diganti.</p>
          </div>
          <x-input name="student_number" label="Nomor Induk (NIP/NPM) (opsional)" value="{{ old('student_number', $user->student_number) }}" />

          @php
            $currentRole = (string) old('role', $user->roles->first()?->name ?? 'Mahasiswa');
            ...
            $jabatanValue = (string) old('jabatan', $user->jabatan ?? '');
          ...
          @php
            $initialUnitId = (string) old('unit_id', (string) ($user->unit_id ?? ''));
            $initialScopes = array_values(array_unique(array_map('intval', (array) old('scoped_unit_ids', $user->unitScopes->pluck('id')->all()))));
          ...
          @php
            $scopedSelected = collect(old('scoped_unit_ids', $user->unitScopes->pluck('id')->map(fn($v)=>(string)$v)->all() ));
          ...
          <div class="au-span-2 au-scoped lg:col-span-2 {{ $showScopes ? '' : 'hidden' }}" data-scoped-units>
            ...
            >
              @foreach(($prodiUnits ?? []) as $u)
                <option value="{{ $u->id }}" @selected($scopedSelected->contains((string)$u->id))>{{ $u->name }}</option>
              @endforeach
            </select>
      ...
      <div class="au-side lg:col-span-5 space-y-4">
        <x-card class="au-photo-card">
          <div class="flex items-center justify-between gap-3 mb-4">
            <div>
              ...
              <div class="text-xs text-muted mt-0.5">Tampilkan ikon yang lebih personal.</div>
            </div>
          </div>

          <div class="au-photo-row flex items-start gap-4">
            <x-user.avatar :user="$user" size="80" class="rounded-2xl" />
            <div class="flex-1 min-w-0">
              <div class="space-y-1">
                <label class="text-sm font-medium" for="profile_photo">Ganti foto profil</label>
        ...
        <x-card class="au-role-card">
          <div class="flex flex-col gap-1 mb-4">
            ...
            <div class="text-xs text-muted">Pilih role yang akan digunakan.</div>
          </div>

          <x-select name="role" label="Role" required>
            @foreach($roles as $r)
              <option value="{{ $r->name }}" @selected(old('role', $user->roles->first()?->name ?? 'Mahasiswa') === $r->name)>{{ $r->name }}</option>
            @endforeach
          </x-select>
        </x-card>
      </div>
