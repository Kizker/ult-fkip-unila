    ...
    <div class="public-container public-footer__inner">
      <div class="public-footer__top">
        <div class="public-footer__brand">
          <a href="{{ route('home') }}" class="public-footer__logo">
          ...
          <div class="public-footer__tagline">{{ __('app.footer_tagline') }}</div>
        </div>
        ...
        <div class="public-footer__cols" aria-label="Tautan">
          <div class="public-footer__col" style="display:flex;flex-direction:column;align-items:flex-start;padding:1rem 1.1rem;border:1px solid rgba(255,255,255,.12);border-radius:18px;background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.03));box-shadow:0 18px 40px rgba(44,7,73,.18);backdrop-filter:blur(10px);">
            ...
            @forelse ($serviceCategories as $category)
              <a class="public-footer__link" href="{{ route('services.index', ['category' => $category->id]) }}" style="display:flex;align-items:center;gap:.65rem;width:100%;padding:.65rem .2rem;border-bottom:1px solid rgba(255,255,255,.08);">
                ...
                <span>{{ app()->getLocale() === 'en' ? ($category->name_en ?? $category->name_id) : $category->name_id }}</span>
              </a>
            @empty
              <a class="public-footer__link" href="{{ route('services.index') }}" style="display:flex;align-items:center;gap:.65rem;width:100%;padding:.65rem .2rem;">
                <span style="display:inline-flex;align-items:center;justify-content:center;width:1.45rem;height:1.45rem;border-radius:999px;background:rgba(255,255,255,.08);color:rgba(255,255,255,.92);font-size:.78rem;line-height:1;">+</span>
                <span>{{ __('app.services') }}</span>
              </a>
            @endforelse
          ...
          <div class="public-footer__col" style="display:flex;flex-direction:column;align-items:flex-start;padding:1rem 1.1rem;border:1px solid rgba(255,255,255,.12);border-radius:18px;background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.03));box-shadow:0 18px 40px rgba(44,7,73,.18);backdrop-filter:blur(10px);">
            <div class="public-footer__title" style="display:flex;align-items:center;gap:.55rem;margin-bottom:.65rem;">
              ...
              <span>{{ __('app.related_links') }}</span>
            </div>
            @foreach ($relatedLinks as $link)
              <a class="public-footer__link" href="{{ $link['url'] }}" target="_blank" rel="noopener noreferrer" style="display:flex;align-items:center;gap:.65rem;width:100%;padding:.65rem .2rem;border-bottom:1px solid rgba(255,255,255,.08);">
                <span style="display:inline-flex;align-items:center;justify-content:center;width:1.45rem;height:1.45rem;border-radius:999px;background:rgba(255,255,255,.08);color:rgba(255,255,255,.92);font-size:.78rem;line-height:1;">↗</span>
      ...
      <div class="public-footer__bottom">
        ...
        <div class="public-footer__meta">
          @foreach ($bottomLinks as $link)
            <a class="public-footer__link" href="{{ $link['url'] }}">{{ $link['label'] }}</a>
          @endforeach
        </div>
      </div>
    </div>
