...
for p in parsed_refs:
    ...
    if not is_book and not url:
        # Fetch DOI
        new_url = fetch_doi(title)
        if new_url:
            url = new_url
            added_dois.append({'authors': auth, 'title': title, 'url': url})
        time.sleep(0.5)
...
with open(r'c:\laragon\www\ult-fkip-unila\docs\skripsi\added_dois.json', 'w', encoding='utf-8') as f:
    json.dump(added_dois, f, indent=2)
...
for ref in formatted_refs:
    ...
    if ref['is_book']:
        ...
        if ref['url']:
            p.add_run(f" {ref['url']}. Diakses pada 7 Juni 2026.").italic = False
