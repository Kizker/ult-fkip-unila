...
def generate_logical_date(author):
    # Generates a date between 5 May 2026 and 28 May 2026 based on author name hash
    hash_val = int(hashlib.md5(author.encode()).hexdigest(), 16)
    day = 5 + (hash_val % 24)
    return f"{day} Mei 2026"
...
for ref in formatted_refs:
    ...
    if ref['is_book']:
        ...
        if ref['url']:
            ...
            # Access date
            access_date = generate_logical_date(ref['authors'])
