...
def set_p(p, text, style, is_highlighted=False, bold=False):
    p.text = ''
    p.style = style
    if text:
        run = p.add_run(text)
...
def repair_doc(doc_path, is_highlighted=False):
    ...
    # 1. Mutate existing paragraphs in-place (631 to 637)
    # Be sure to match exact indices based on previous run!
    p631 = doc.paragraphs[631]
    set_p(p631, '3.4 Tempat dan Waktu Penelitian', 'Heading 2', is_highlighted)
    
    p632 = doc.paragraphs[632]
    set_p(p632, narasi1, 'Paragraph', is_highlighted)
    
    p633 = doc.paragraphs[633]
    set_p(p633, narasi2, 'Paragraph', is_highlighted)
    
    p634 = doc.paragraphs[634]
    set_p(p634, 'Tabel 3.1 Waktu Pelaksanaan Penelitian', 'Normal', is_highlighted, bold=True)
    p634.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    # Insert Table right after 634
    p634._p.addnext(table._tbl)
    ...
    p635 = doc.paragraphs[635]
    set_p(p635, '', 'Normal', is_highlighted=False)
    
    p636 = doc.paragraphs[636]
    set_p(p636, '3.5 Subjek Penelitian dan Sampling', 'Heading 2', is_highlighted=False)
    
    p637 = doc.paragraphs[637]
    set_p(p637, '', 'Normal', is_highlighted=False)
    
    # 638 is already subjek narrative
    # Need to insert "Validator Ahli" before 639
    p639 = doc.paragraphs[639]
    new_p = p639.insert_paragraph_before('Validator Ahli', style='List Paragraph')
    # Because insert_paragraph_before might lose some run properties if we just set text
    if not new_p.runs:
        run = new_p.add_run('Validator Ahli')
    
    doc.save(doc_path)
    print(f"Successfully repaired {doc_path}")
...
if __name__ == '__main__':
    ...
    repair_doc(clean_path, is_highlighted=False)
    repair_doc(highlighted_path, is_highlighted=True)
