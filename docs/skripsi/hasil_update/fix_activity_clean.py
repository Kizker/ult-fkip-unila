...
# 1. GENERATE MONOCHROME MERMAID IMAGES
diagrams = [
    {
...
# Note: Changed `((Selesai))` to `(((Selesai)))` to render as double circle (end node in Mermaid).
# Start node `((Mulai))` stays as a circle. Wait, in standard mermaid, `((*))` is start, `(((*)))` is not standard.
# Standard mermaid uses:
# A((Mulai)) -> normal circle
# B(((Selesai))) -> double circle
...
def process_doc(filepath, is_highlighted):
    ...
    # Clear all paragraphs between start_idx (exclusive) and end_idx (exclusive)
    for i in range(start_idx + 1, end_idx):
        doc.paragraphs[i].clear()
        
    # We will insert everything BEFORE the end_idx paragraph
    # (Since we cleared the paragraphs above it, inserting before it is safe and preserves the order)
    ...
    for item in diagrams:
        ...
        run_img.add_picture(item["file"], width=Inches(4.5)) # Reduced width for better scale
        ...
        run_cap.font.bold = True # Often captions are bold, but let's stick to standard unbold if Ajeng didn't use bold
